...
import { iVerse as oldVerse } from './verse';
...
export class Verse implements iVerse {
  static fromOldVerse(data: oldVerse): Verse {
    return new Verse({
      path: {
        book: data.book,
  ...
  get book(): string {
    if ((this.path as string) !== undefined) {
      return this.path as string;
    } else {
      return (this.path as BiblePath).book;
    }
  }
